...
export default function KennelCardPage() {
  ...
  // Uses retry logic to handle cases where SVG element isn't mounted yet
  useEffect(() => {
    if (!animal?.microchipNumber || cardTemplate !== "staff") return;
    
    let attempts = 0;
    const maxAttempts = 10;
    let timeoutId: ReturnType<typeof setTimeout>;
    
    const tryGenerateBarcode = () => {
      ...
      if (attempts < maxAttempts) {
        timeoutId = setTimeout(tryGenerateBarcode, 100);
      }
    };
    
    timeoutId = setTimeout(tryGenerateBarcode, 50);
      ...
      <CardContent className="p-6 space-y-4 kennel-card-content flex-1 flex flex-col">
        ...
        <div className="animal-info-section flex-grow flex flex-col">
          <div className="p-4 flex-grow flex flex-col">
            {/* Good With Icons Row - all on one line */}
            <div className="flex gap-2 mb-4">
              {/* Kids */}
              <div className="flex items-center gap-1.5 border-2 rounded-lg px-2 py-1.5" data-testid="staff-indicator-child-friendly">
                <Baby className="w-5 h-5 text-muted-foreground" />
                <span className={`${fontSize === 'small' ? 'text-xs' : 'text-sm'} font-medium`}>Kids</span>
                {animal.childFriendly === true && <Check className="w-4 h-4 text-green-600" />}
                {animal.childFriendly === false && <XIcon className="w-4 h-4 text-red-600" />}
                {animal.childFriendly == null && <span className="text-sm text-muted-foreground">?</span>}
              </div>
              
              {/* Cats */}
              <div className="flex items-center gap-1.5 border-2 rounded-lg px-2 py-1.5" data-testid="staff-indicator-cat-friendly">
                <Cat className="w-5 h-5 text-muted-foreground" />
                <span className={`${fontSize === 'small' ? 'text-xs' : 'text-sm'} font-medium`}>Cats</span>
                {animal.catFriendly === true && <Check className="w-4 h-4 text-green-600" />}
                {animal.catFriendly === false && <XIcon className="w-4 h-4 text-red-600" />}
                {animal.catFriendly == null && <span className="text-sm text-muted-foreground">?</span>}
              </div>
              
              {/* Dogs */}
              <div className="flex items-center gap-1.5 border-2 rounded-lg px-2 py-1.5" data-testid="staff-indicator-dog-friendly">
                <Dog className="w-5 h-5 text-muted-foreground" />
                <span className={`${fontSize === 'small' ? 'text-xs' : 'text-sm'} font-medium`}>Dogs</span>
                {animal.dogFriendly === true && <Check className="w-4 h-4 text-green-600" />}
                {animal.dogFriendly === false && <XIcon className="w-4 h-4 text-red-600" />}
                {animal.dogFriendly == null && <span className="text-sm text-muted-foreground">?</span>}
              </div>

              {/* Heartworm Status */}
              <div className="flex items-center gap-1.5 border-2 rounded-lg px-2 py-1.5" data-testid="staff-indicator-heartworm">
                <Heart className="w-5 h-5 text-muted-foreground" />
                <span className={`${fontSize === 'small' ? 'text-xs' : 'text-sm'} font-medium`}>HW</span>
                {animal.heartwormPositive === true && <span className="text-sm font-bold text-red-600">+</span>}
                {animal.heartwormPositive === false && <span className="text-sm font-bold text-green-600">-</span>}
                {animal.heartwormPositive == null && <span className="text-sm text-muted-foreground">?</span>}
              </div>

              {/* Spay/Neuter Status */}
              <div className="flex items-center gap-1.5 border-2 rounded-lg px-2 py-1.5" data-testid="staff-indicator-altered">
                ...
                {(animal.neuterStatus === 'spayed' || animal.neuterStatus === 'neutered') && <Check className="w-4 h-4 text-green-600" />}
                {animal.neuterStatus === 'intact' && <XIcon className="w-4 h-4 text-orange-500" />}
              </div>
            </div>
            ...
            <div className={`grid grid-cols-2 gap-x-4 gap-y-6 ${getFontSizeClass()} flex-grow`}>
              ...
              {(animal.intakeSource === 'stray' || animal.status === 'stray_hold' || animal.locationFound || animal.strayHoldUntil) && (
                <>
                  <div>
