...
function AnimalSelectorDialog({ open, onClose, buildingId, rowId, position, currentAnimalId }: AnimalSelectorDialogProps) {
  ...
  // Include animals that don't have a COMPLETE kennel assignment (missing building, row, OR position)
  const availableAnimals = animals.filter(animal => 
    animal.id === currentAnimalId || 
    (
      (!animal.kennelBuildingId || !animal.kennelRowId || animal.kennelPosition === null || animal.kennelPosition === undefined) && 
      animal.status !== 'adopted' && 
      animal.status !== 'deceased' &&
      animal.status !== 'merged'
    )
  );
