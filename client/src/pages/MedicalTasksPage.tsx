...
export default function MedicalTasksPage() {
  ...
  const [administerTime, setAdministerTime] = useState("");
  ...
  const administerMutation = useMutation({
    mutationFn: async ({ doseId, notes, administeredAt }: { doseId: string; notes?: string; administeredAt?: string }) => {
      return await apiRequest('PATCH', '/api/medical/doses/' + doseId + '/administer', { notes, administeredAt });
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      const message = error instanceof Error ? error.message : "Failed to administer dose. Please try again.";
      toast({
        title: "Error",
        description: message,
        variant: "destructive",
  ...
  const confirmAdminister = () => {
    if (selectedDose) {
      administerMutation.mutate({
        ...
        administeredAt: administerTime ? new Date(administerTime).toISOString() : undefined,
      });
    }
  };
              ...
              {selectedDose && (
                <>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="administer-time">Date & Time Given</Label>
                    <Input
                      id="administer-time"
                      type="datetime-local"
                      ...
                      data-testid="input-administer-time"
                    />
                    <p className="text-xs text-muted-foreground">
                      Defaults to now. Change if you're back-charting a dose given earlier.
                    </p>
                  </div>
                  <div className="space-y-2">
