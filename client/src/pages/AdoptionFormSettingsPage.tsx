...
function SortableFieldItem({ 
  ...
  field: AdoptionFormField; 
  onEdit: (field: AdoptionFormField) => void; 
...
export default function AdoptionFormSettingsPage() {
  const { toast } = useToast();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingField, setEditingField] = useState<AdoptionFormField | null>(null);
  const [introText, setIntroText] = useState("");
  const [showCustomText, setShowCustomText] = useState(false);

  const { data, isLoading } = useQuery<{ fields: AdoptionFormField[] }>({
  ...
  const saveIntroMutation = useMutation({
    mutationFn: async (text: string) => {
      return await apiRequest('PUT', '/api/form-settings/adoption', { introText: text || null });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/form-settings', 'adoption'] });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormFieldFormData) => {
      ...
      return await apiRequest('POST', '/api/adoption-form-fields', payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/adoption-form-fields'] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<FormFieldFormData> }) => {
      ...
      return await apiRequest('PATCH', `/api/adoption-form-fields/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/adoption-form-fields'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiRequest('DELETE', `/api/adoption-form-fields/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/adoption-form-fields'] });
  ...
  const reorderMutation = useMutation({
    mutationFn: async (fieldIds: string[]) => {
      return await apiRequest('POST', '/api/adoption-form-fields/reorder', { fieldIds });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/adoption-form-fields'] });
    },
  ...
  const handleOpenDialog = (field?: AdoptionFormField) => {
    if (field) {
    ...
    <div className="container max-w-4xl p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add introductory text that appears at the top of the adoption application form.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Textarea
            ...
            placeholder="Welcome to our adoption application! Please fill out this form to begin the adoption process..."
            rows={4}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <p className="text-muted-foreground mb-4">
                No custom questions yet. Add questions to customize your adoption application form.
              </p>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Configure a custom question for your adoption application form
            </DialogDescription>
          </DialogHeader>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        placeholder="e.g., Do you have a fenced yard?"
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        placeholder="e.g., Tell us about your yard..."
