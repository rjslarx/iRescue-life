...
export default function TenantsPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const platformFeeValue = formData.get('platformFeePercent');
    const data = {
      subdomain: formData.get('subdomain'),
      name: formData.get('name'),
      contactEmail: formData.get('contactEmail'),
      isActive: formData.get('isActive') === 'on',
      platformFeePercent: platformFeeValue ? parseFloat(platformFeeValue as string) : null,
    };
                      ...
                      <CardContent>
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex items-center justify-between text-xs text-muted-foreground pt-2">
                            ...
                            <span>
                              Fee: {tenant.platformFeePercent !== null ? `${tenant.platformFeePercent}%` : 'Default'}
                            </span>
                          </div>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max="100"
                  step="0.1"
                  placeholder="e.g., 5.0 (leave empty for tier default)"
                  defaultValue={editingTenant?.platformFeePercent ?? ''}
                  data-testid="input-platform-fee"
                />
                <p className="text-xs text-muted-foreground">
                  Custom platform fee for this tenant. Leave empty to use tier-based default (Free: 5%, Professional: varies).
                </p>
              </div>
