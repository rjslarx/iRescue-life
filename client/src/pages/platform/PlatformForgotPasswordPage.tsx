...
export default function PlatformForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/platform/forgot-password', {
        method: 'POST',
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-2xl">Reset Platform Admin Password</CardTitle>
          </div>
          <CardDescription>
            Enter your platform admin email address and we'll send you a link to reset your password
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="flex justify-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href="/platform/login" className="text-primary hover:underline" data-testid="link-login">
              Back to login
            </Link>
          </p>
        </CardFooter>
