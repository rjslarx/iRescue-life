...
export default function AdopterLoginPage() {
  ...
  const magicLoginMutation = useMutation({
    mutationFn: async (loginToken: string) => {
      const response = await apiRequest("POST", "/api/adopter/magic-login", { token: loginToken });
  ...
  const requestLinkMutation = useMutation({
    mutationFn: async (userEmail: string) => {
      const response = await apiRequest("POST", "/api/adopter/request-magic-link", { email: userEmail });
      return await response.json();
    },
    onSuccess: () => {
