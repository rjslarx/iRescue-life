...
import { FileText, Plus, Edit, Trash2, Eye, Star, Copy, ChevronDown, GripVertical, Type, LayoutTemplate, AlertCircle, Heart, PawPrint } from "lucide-react";
...
type ContractType = 'adoption' | 'foster';
...
function generateHtmlFromSections(sections: ContractTemplateSection[], contractType: ContractType): string {
  ...
  const title = contractType === 'adoption' ? 'Adoption Contract' : 'Foster Care Agreement';
...
export default function ContractTemplatesPage() {
  ...
  const currentMergeFields = activeContractType === 'adoption' ? ADOPTION_MERGE_FIELDS : FOSTER_MERGE_FIELDS;
  const currentDefaultSections = activeContractType === 'adoption' ? DEFAULT_ADOPTION_SECTIONS : DEFAULT_FOSTER_SECTIONS;
  ...
  const templates = activeContractType === 'adoption' ? adoptionTemplates : fosterTemplates;
  const isLoading = activeContractType === 'adoption' ? adoptionLoading : fosterLoading;

  // API endpoint based on contract type
  const getApiEndpoint = (type: ContractType) => 
    type === 'adoption' ? '/api/contract-templates' : '/api/foster-contract-templates';
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `${activeContractType === 'adoption' ? 'Adoption' : 'Foster'} contract template has been created successfully`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `${activeContractType === 'adoption' ? 'Adoption' : 'Foster'} contract template has been updated successfully`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `${activeContractType === 'adoption' ? 'Adoption' : 'Foster'} contract template has been deleted successfully`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const setDefaultMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `This template is now the default for new ${activeContractType === 'adoption' ? 'adoptions' : 'foster placements'}`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleTabChange = (value: string) => {
    ...
    setGuidedSections(newType === 'adoption' ? DEFAULT_ADOPTION_SECTIONS : DEFAULT_FOSTER_SECTIONS);
  };

  if (adoptionLoading && fosterLoading) {
    return (
      <DashboardLayout
  ...
  return (
    <DashboardLayout
      title="Contract Templates"
      description="Manage adoption and foster contract templates with customizable merge fields"
      breadcrumbs={[
        { label: "Administration" },
        { label: "Contract Templates" }
      ]}
        ...
        <div className="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Manage adoption and foster contract templates with customizable merge fields
            </p>
          </div>
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <form onSubmit={createForm.handleSubmit(onSubmitCreate)}>
              <DialogHeader>
                <DialogTitle>
                  Create {activeContractType === 'adoption' ? 'Adoption' : 'Foster'} Contract Template
                </DialogTitle>
                <DialogDescription>
                  Create a new {activeContractType === 'adoption' ? 'adoption' : 'foster care'} contract template with merge fields.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4 py-4">
                ...
                {/* Editor Mode Tabs */}
                <Tabs value={editorMode} onValueChange={(v) => setEditorMode(v as 'richText' | 'guided')}>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Create your first {activeContractType === 'adoption' ? 'adoption' : 'foster care'} contract template to get started
            </p>
