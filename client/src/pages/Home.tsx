...
export default function Home() {
  ...
  // publicOnly=true ensures only available/foster animals are returned even for authenticated users
  const { data: animalsData, isLoading: isLoadingAnimals } = useQuery<{ animals: Animal[] }>({
    queryKey: ['/api/animals?publicOnly=true', tenantId],
  });
  ...
  const jotformAdoptionUrl = (tenant as any)?.jotformAdoptionUrl;
  const jotformVolunteerUrl = isLiteTier ? (tenant as any)?.jotformVolunteerUrl : null;
  const jotformFosterUrl = isLiteTier ? (tenant as any)?.jotformFosterUrl : null;
  const jotformSurrenderUrl = isLiteTier ? (tenant as any)?.jotformSurrenderUrl : null;

  const handleAdopt = (animal: Animal) => {
    if (isLiteTier && jotformAdoptionUrl) {
      try {
        const url = new URL(jotformAdoptionUrl);
        url.searchParams.set('animalName', animal.name);
        url.searchParams.set('animalBreed', animal.breed);
        url.searchParams.set('animalSpecies', animal.species);
        url.searchParams.set('animalId', animal.id.toString());
        window.open(url.toString(), '_blank', 'noopener,noreferrer');
      } catch {
        window.open(jotformAdoptionUrl, '_blank', 'noopener,noreferrer');
      }
    } else {
        ...
        <div className="container px-6 sm:px-8">
          <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 md:grid-cols-4">
            ...
            <div>
              ...
              <div className="text-sm flex flex-col gap-2">
                {jotformVolunteerUrl ? (
                  <a href={jotformVolunteerUrl} target="_blank" rel="noopener noreferrer" data-testid="link-footer-volunteer">
                    <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                      Volunteer
                    </span>
                  </a>
                ...
                {jotformFosterUrl ? (
                  <a href={jotformFosterUrl} target="_blank" rel="noopener noreferrer" data-testid="link-footer-foster">
                    <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                      Become a Foster
                    </span>
                  </a>
                ...
                {jotformSurrenderUrl ? (
                  <a href={jotformSurrenderUrl} target="_blank" rel="noopener noreferrer" data-testid="link-footer-surrender">
                    <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                      Surrender an Animal
                    </span>
                  </a>
          ...
          <div className="mt-8 pt-8 border-t text-center text-sm text-muted-foreground space-y-2">
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <a href={tenant?.privacyPolicyUrl || "/platform/privacy"} target={tenant?.privacyPolicyUrl ? "_blank" : undefined} rel={tenant?.privacyPolicyUrl ? "noopener noreferrer" : undefined} className="hover:text-foreground transition-colors cursor-pointer" data-testid="link-footer-privacy">
                Privacy Policy
              </a>
              <span className="text-muted-foreground/50">•</span>
              <a href={tenant?.termsOfServiceUrl || "/platform/terms"} target={tenant?.termsOfServiceUrl ? "_blank" : undefined} rel={tenant?.termsOfServiceUrl ? "noopener noreferrer" : undefined} className="hover:text-foreground transition-colors cursor-pointer" data-testid="link-footer-terms">
                Terms of Service
              </a>
            </div>
            <p>
