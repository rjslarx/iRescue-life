...
const ACTIVE_POOL_VISIBLE_LIMIT = 10;
...
function StatusColumn({ status, items, pipelineType, onItemClick, showChevron, triggersAutomation, columnClassName }: StatusColumnWithChevronProps) {
  const [showAll, setShowAll] = useState(false);
  let filteredItems = items.filter(item => item.status === status);
  
  if (status === 'active_pool') {
  ...
  const isCapped = status === 'active_pool' && totalCount > ACTIVE_POOL_VISIBLE_LIMIT && !showAll;
  const visibleItems = isCapped ? filteredItems.slice(0, ACTIVE_POOL_VISIBLE_LIMIT) : filteredItems;
  
  return (
    <div className="flex items-stretch" data-testid={`status-column-wrapper-${pipelineType}-${status}`}>
      <div 
        className={`${columnClassName || 'w-[200px]'} flex flex-col rounded-lg overflow-hidden border bg-card`} 
      ...
      >
        <div className="bg-slate-700 dark:bg-slate-800 text-white px-3 py-2 text-sm font-medium">
          {stageLabels[status] || status} <span className="opacity-80" data-testid={`text-count-${pipelineType}-${status}`}>({totalCount})</span>
        </div>
        <div className="p-3 space-y-2 h-[200px] overflow-y-auto bg-muted/20">
          {filteredItems.length === 0 ? (
            ...
            <>
              ...
              {status === 'active_pool' && totalCount > ACTIVE_POOL_VISIBLE_LIMIT && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full text-xs text-muted-foreground"
                  onClick={() => setShowAll(!showAll)}
                  data-testid={`button-toggle-active-pool-${pipelineType}`}
                >
      ...
      {showChevron && (
        <div className="flex items-center justify-center w-6 flex-shrink-0" title={triggersAutomation ? "Email automation triggers here" : undefined}>
          {triggersAutomation ? (
            <Zap className="h-4 w-4 text-amber-500" data-testid={`icon-automation-${pipelineType}-${status}`} />
          ) : (
...
export default function PipelineManager({ activeTab, onTabChange, permissions }: PipelineManagerProps) {
  ...
  const volunteerItems: PipelineItemData[] = (volunteers || []).map(v => ({
    ...
    updatedAt: (v as any).updatedAt || v.createdAt,
  }));
          ...
          <Tabs value={currentTab} onValueChange={handleTabChange} className="w-full">
            ...
            {canViewAdoptions && (
              ...
              {adoptionItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center h-[280px]" data-testid="empty-state-adoptions">
                ...
                <>
                  <ScrollArea className="h-[280px] lg:hidden">
                    <div className="space-y-2">
                  ...
                  <div className="hidden lg:block overflow-x-auto pb-2 max-h-[280px]" data-testid="adoption-pipeline-scroll">
                    <div 
                      className="flex items-stretch" 
                      style={{ minWidth: `${adoptionStages.length * 224}px` }}
                    >
                      {adoptionStages.map((stage, index) => (
                        <div key={stage} className="flex-shrink-0">
                          <StatusColumn 
                            ...
                            triggersAutomation={stage === "home_visit"}
                          />
                        </div>
                      ))}
                    </div>
            ...
            {canViewFosters && (
              ...
              {fosterItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center h-[280px]" data-testid="empty-state-fosters">
                ...
                <>
                  <ScrollArea className="h-[280px] lg:hidden">
                    <div className="space-y-2">
                      {[...fosterItems].sort((a, b) => {
                        if (a.status === 'active_pool' && b.status === 'active_pool') {
                          const dateA = a.updatedAt ? new Date(a.updatedAt).getTime() : 0;
                          const dateB = b.updatedAt ? new Date(b.updatedAt).getTime() : 0;
                          return dateB - dateA;
                        }
                  ...
                  <div className="hidden lg:block overflow-x-auto pb-2 max-h-[280px]" data-testid="foster-pipeline-scroll">
                    <div 
                      className="flex items-stretch" 
                      style={{ minWidth: `${fosterPipelineStatuses.length * 224}px` }}
                    >
            ...
            {canViewVolunteers && (
              ...
              {volunteerItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center h-[280px]" data-testid="empty-state-volunteers">
                ...
                <>
                  ...
                  <div className="hidden lg:block pb-2 max-h-[280px]" data-testid="volunteer-pipeline-scroll">
            ...
            {canViewIntake && (
              ...
              {intakeItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center h-[280px]" data-testid="empty-state-intake">
                ...
                <>
                  ...
                  <div className="hidden lg:block overflow-x-auto pb-2 max-h-[280px]" data-testid="intake-pipeline-scroll">
                    <div 
                      className="flex items-stretch"
                      style={{ minWidth: `${surrenderStatuses.length * 224}px` }}
                    >
                      {surrenderStatuses.map((status, index) => (
                        <div key={status} className="flex-shrink-0">
                          <StatusColumn 
                            ...
                            triggersAutomation={status === 'waitlist'}
                          />
                        </div>
                      ))}
                    </div>
