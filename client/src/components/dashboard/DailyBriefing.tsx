...
interface DailyBriefingData {
  ...
  summary: {
    ...
    vetEventsTodayCount: number;
    vetEventsTomorrowCount: number;
  };
}
...
export function DailyBriefing() {
      ...
      <Card className="h-full" data-testid="card-daily-briefing">
        ...
        <CardContent className="flex items-center justify-center py-8">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm" data-testid="text-daily-briefing-error">Unable to load briefing</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  const hasVetEventsToday = data && data.today.vetEvents && data.today.vetEvents.length > 0;
  const hasTomorrowItems = data && (
    ...
    (data.tomorrow.vetEvents && data.tomorrow.vetEvents.length > 0)
  );

  const isEmpty = !hasUrgent && !hasMedicalToday && !hasVetEventsToday && !hasTomorrowItems && 
        ...
        <ScrollArea className="h-full max-h-[420px] px-4 pb-4">
          {isEmpty ? (
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {data?.today.surgeries.map((surgery) => (
                      <Link 
                        key={surgery.id} 
                        href="/dashboard/medical-pipeline?tab=surgery"
                        className="block"
                    ...
                    {data?.today.transports.map((transport) => (
                      <Link 
                        key={transport.id} 
                        href={`/dashboard/transport/${transport.id}/active`}
                        className="block"
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-semibold">Vet Appointments</span>
                      <Badge variant="secondary" className="text-xs">
                        {data?.today.vetEvents.length}
                      </Badge>
                    </div>
                    <div className="space-y-1">
                      {data?.today.vetEvents.map((event) => (
                        <Link
                          key={event.id}
                          href={`/dashboard/animals/${event.animalId}/medical`}
                          className="block"
                          data-testid={`link-vet-event-${event.id}`}
                        >
                          <div className="flex items-center gap-2 p-2 rounded-md bg-red-500/10 hover-elevate cursor-pointer">
                            <Stethoscope className="h-3 w-3 text-red-600 flex-shrink-0" />
                            <div className="flex-1 min-w-0">
              ...
              {data?.today.calendar && data.today.calendar.length > 0 && (
                <>
                  {(hasUrgent || hasMedicalToday || hasVetEventsToday) && <Separator />}
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {data?.tomorrow.vetEvents?.map((event) => (
                        <Link
                          key={event.id}
                          href={`/dashboard/animals/${event.animalId}/medical`}
                          className="block"
                          data-testid={`link-tomorrow-vet-event-${event.id}`}
                        >
                          <div className="flex items-center gap-2 p-2 rounded-md bg-red-500/5 hover-elevate cursor-pointer">
                            <Stethoscope className="h-3 w-3 text-red-600/70 flex-shrink-0" />
                            <div className="flex-1 min-w-0">
