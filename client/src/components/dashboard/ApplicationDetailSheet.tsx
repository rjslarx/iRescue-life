...
import { ChangeAnimalDialog } from "@/components/ChangeAnimalDialog";
import { ApproveAndSendAgreementDialog } from "@/components/ApproveAndSendAgreementDialog";
import { MoveAndSendFosterAgreementDialog } from "@/components/MoveAndSendFosterAgreementDialog";
import { MoveAndSendVolunteerWaiverDialog } from "@/components/MoveAndSendVolunteerWaiverDialog";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export default function ApplicationDetailSheet({
  ...
  const [showFosterAgreementDialog, setShowFosterAgreementDialog] = useState(false);
  const [showVolunteerWaiverDialog, setShowVolunteerWaiverDialog] = useState(false);
  const [showChangeAnimalDialog, setShowChangeAnimalDialog] = useState(false);

  const { data: tenantData } = useQuery<{ name: string }>({
    queryKey: ['/api/tenant'],
    enabled: isOpen,
  });
  ...
  const prepareIntakeMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`/api/surrender/${data?.id}/prepare-intake`, {
        credentials: 'include',
      });
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.error || 'Failed to prepare intake data');
      }
      return response.json();
    },
    onSuccess: (result) => {
      onClose();
      sessionStorage.setItem('surrenderPrefill', JSON.stringify(result));
      setLocation(`/dashboard/animals?from_surrender=${result.surrenderRequestId}`);
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error?.message || "Failed to prepare intake data.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleStatusChange = (newStatus: string) => {
    // For adoption applications moving to "approved", show the approval dialog
    if (type === "adoption" && newStatus === "approved") {
      setShowApprovalDialog(true);
      return;
    }
    // For foster applications moving to "agreement", show the foster agreement dialog
    if (type === "foster" && newStatus === "agreement") {
      setShowFosterAgreementDialog(true);
      return;
    }
    // For volunteer applications moving to "waiver_needed", show the volunteer waiver dialog
    if (type === "volunteer" && newStatus === "waiver_needed") {
      setShowVolunteerWaiverDialog(true);
      return;
    }
  ...
  const handleAdvance = () => {
    if (nextStage) {
      // For adoption applications advancing to "approved", show the approval dialog
      if (type === "adoption" && nextStage === "approved") {
        setShowApprovalDialog(true);
        return;
      }
      // For foster applications advancing to "agreement", show the foster agreement dialog
      if (type === "foster" && nextStage === "agreement") {
        setShowFosterAgreementDialog(true);
        return;
      }
      // For volunteer applications advancing to "waiver_needed", show the volunteer waiver dialog
      if (type === "volunteer" && nextStage === "waiver_needed") {
        setShowVolunteerWaiverDialog(true);
        return;
      }
  ...
  const renderFullDetailsSection = () => {
    if (type === "adoption") {
      ...
      const orphanedAdoptionEntries = Object.entries(customResponses).filter(([id]) => !knownFieldIds.has(id) && !dealbreakerKeys.includes(id));
      const otherFields = allFields.filter(f => !dealbreakerKeys.includes(f.id));
      ...
      return (
        <div className="space-y-2 pt-2" data-testid="full-details-adoption">
          {adoptionData.animal && (
            <div className="flex items-center justify-between gap-2 py-1.5 border-b border-border/50">
              ...
              <div className="flex items-center gap-1">
                <span className="text-sm font-medium" data-testid="summary-value-animal">{adoptionData.animal.name}</span>
                {(user?.activeRole === "admin" || user?.activeRole === "manager" || user?.activeRole === "staff") && (
                  <Button
                    size="sm"
                    variant="outline"
          ...
          <div className="flex items-center justify-between gap-2 py-1.5 border-b border-border/50">
            ...
            <div className="flex items-center gap-1">
              <span className="text-sm font-medium" data-testid="summary-value-phone">{adoptionData.applicantPhone}</span>
              <ContactButton
                mode="manual"
                phoneNumber={adoptionData.applicantPhone}
                recipientName={adoptionData.applicantName}
                messageBody={`Hi ${adoptionData.applicantName}, this is ${tenantData?.name || "our rescue"} regarding your application${adoptionData.animal ? ` for ${adoptionData.animal.name}` : ""}. `}
                label="Text Applicant"
                size="sm"
                variant="outline"
                data-testid="button-text-applicant-adoption"
              />
            </div>
          </div>
          <SummaryItem 
          ...
          {dealbreakerKeys.some(key => customResponses[key] !== undefined && customResponses[key] !== null) && (
            <>
              <div className="border-t pt-2 mt-2" />
              <p className="text-sm font-medium text-muted-foreground">Housing & Pet Info</p>
              {dealbreakerKeys.map((key) => {
                const val = customResponses[key];
                const isEmpty = val === undefined || val === null || val === '';
                return (
                  <SummaryItem 
                    key={key}
                    label={formatFieldLabel(key)} 
                    value={isEmpty ? 'Not provided' : formatFieldValue(val)}
                  />
                );
              })}
            </>
          ...
          {(otherFields.length > 0 || orphanedAdoptionEntries.length > 0) && (
            <>
              <div className="border-t pt-2 mt-2" />
              ...
              {otherFields.map((field) => {
                ...
                if (fieldType === 'photo' && !isEmpty && typeof val === 'string' && (val.startsWith('http') || val.startsWith('/objects/') || val.startsWith('objects/'))) {
                  return (
                    <div key={field.id} className="mb-3">
                      ...
                      <img 
                        ...
                        data-testid={`img-custom-response-${field.id}`}
                      />
                    </div>
                  );
                }
              ...
              {orphanedAdoptionEntries.map(([fieldId, value]) => {
                const isEmpty = value === undefined || value === null || value === '';
                return (
                  <SummaryItem 
                    key={fieldId}
                    label={formatFieldLabel(fieldId)} 
                    value={isEmpty ? 'Not provided' : formatFieldValue(value)}
                  />
                );
              })}
            </>
    ...
    if (type === "intake") {
      ...
      return (
        <div className="space-y-2 pt-2" data-testid="full-details-intake">
          ...
          <SummaryItem label="Address" value={[intakeData.ownerAddress, intakeData.ownerCity, intakeData.ownerState, intakeData.ownerZip].filter(Boolean).join(', ') || 'Not provided'} />
          <SummaryItem 
            label="Submitted" 
          ...
          {intakeData.dogSize && (
            <SummaryItem label="Size" value={intakeData.dogSize} />
          )}
          ...
          {(() => {
            const responses = intakeData.customResponses || {};
            const knownFieldIds = new Set(allFields.map(f => f.id));
            const orphanedEntries = Object.entries(responses).filter(([id]) => !knownFieldIds.has(id));
            const hasContent = allFields.length > 0 || orphanedEntries.length > 0;
            if (!hasContent) return null;

            const renderResponseItem = (fieldId: string, label: string, fieldType: string, value: any) => {
              const isEmpty = value === undefined || value === null || value === '';
              if (fieldType === 'photo' && !isEmpty && typeof value === 'string' && (value.startsWith('http') || value.startsWith('/objects/') || value.startsWith('objects/'))) {
                return (
              ...
              const displayValue = isEmpty ? 'Not provided' : formatFieldValue(value);
              return <SummaryItem key={fieldId} label={label} value={displayValue} />;
            };
            ...
            return (
              <>
                ...
                {allFields.map(field => renderResponseItem(field.id, field.label, field.fieldType || 'text', responses[field.id]))}
                {orphanedEntries.map(([fieldId, value]) => renderResponseItem(fieldId, formatFieldLabel(fieldId), 'text', value))}
              </>
            );
          })()}
          {intakeData.notes && (
      ...
      return (
        <div className="space-y-2 pt-2" data-testid="full-details-foster">
          ...
          <div className="flex items-center justify-between gap-2 py-1.5 border-b border-border/50">
            <span className="text-sm text-muted-foreground">Phone</span>
            <div className="flex items-center gap-1">
              <span className="text-sm font-medium" data-testid="summary-value-phone-foster">{fosterData.applicantPhone}</span>
              <ContactButton
                mode="manual"
                phoneNumber={fosterData.applicantPhone}
                recipientName={fosterData.applicantName}
                messageBody={`Hi ${fosterData.applicantName}, this is ${tenantData?.name || "our rescue"} regarding your foster application. `}
                label="Text Applicant"
                size="sm"
                variant="outline"
                data-testid="button-text-applicant-foster"
              />
            </div>
          </div>
          <SummaryItem 
          ...
          {(() => {
            const responses = fosterData.customResponses || {};
            const knownIds = new Set(allFields.map(f => f.id));
            const orphaned = Object.entries(responses).filter(([id]) => !knownIds.has(id));
            if (allFields.length === 0 && orphaned.length === 0) return null;

            const renderItem = (fieldId: string, label: string, fieldType: string, value: any) => {
              const isEmpty = value === undefined || value === null || value === '';
              if (fieldType === 'photo' && !isEmpty && typeof value === 'string' && (value.startsWith('http') || value.startsWith('/objects/') || value.startsWith('objects/'))) {
                return (
                  <div key={fieldId} className="mb-3">
              ...
              return <SummaryItem key={fieldId} label={label} value={isEmpty ? 'Not provided' : formatFieldValue(value)} />;
            };
            ...
            return (
              <>
                <div className="border-t pt-2 mt-2" />
                <p className="text-sm font-medium text-muted-foreground">Additional Questions</p>
                {allFields.map(field => renderItem(field.id, field.label, field.fieldType || 'text', responses[field.id]))}
                {orphaned.map(([fieldId, value]) => renderItem(fieldId, formatFieldLabel(fieldId), 'text', value))}
              </>
            );
          })()}
          {fosterData.notes && (
      ...
      return (
        <div className="space-y-2 pt-2" data-testid="full-details-volunteer">
          ...
          <div className="flex items-center justify-between gap-2 py-1.5 border-b border-border/50">
            <span className="text-sm text-muted-foreground">Phone</span>
            <div className="flex items-center gap-1">
              <span className="text-sm font-medium" data-testid="summary-value-phone-volunteer">{volunteerData.applicantPhone}</span>
              <ContactButton
                mode="manual"
                phoneNumber={volunteerData.applicantPhone}
                recipientName={volunteerData.applicantName}
                messageBody={`Hi ${volunteerData.applicantName}, this is ${tenantData?.name || "our rescue"} regarding your volunteer application. `}
                label="Text Applicant"
                size="sm"
                variant="outline"
                data-testid="button-text-applicant-volunteer"
              />
            </div>
          </div>
          <SummaryItem 
          ...
          {(() => {
            const responses = volunteerData.customResponses || {};
            const knownIds = new Set(allFields.map(f => f.id));
            const orphaned = Object.entries(responses).filter(([id]) => !knownIds.has(id));
            if (allFields.length === 0 && orphaned.length === 0) return null;
  ...
  return (
    <>
          ...
          <div className="p-4 border-t flex flex-col gap-2 mt-auto">
            {type === "intake" && currentStatus === "scheduled" && (
              <Button
                variant="default"
                className="w-full"
                onClick={() => prepareIntakeMutation.mutate()}
                disabled={prepareIntakeMutation.isPending}
                data-testid="button-finalize-intake"
              >
                <PawPrint className="h-4 w-4 mr-1" />
                {prepareIntakeMutation.isPending ? "Preparing Intake..." : "Approve & Review Intake"}
              </Button>
            )}
            <div className="flex gap-2">
              <Button
                ...
                disabled={updateMutation.isPending || prepareIntakeMutation.isPending || ["denied", "rejected", "declined", "adopted", "intaken"].includes(currentStatus)}
              ...
              <Button
                ...
                disabled={updateMutation.isPending || prepareIntakeMutation.isPending || !nextStage}
      ...
      {/* Approve and Send Agreement Dialog for adoption applications */}
      {type === "adoption" && data && (
        <ApproveAndSendAgreementDialog
          ...
          application={data ? {
            id: data.id,
            applicantName: (data as AdoptionData).applicantName,
            applicantEmail: (data as AdoptionData).applicantEmail,
            applicantPhone: (data as AdoptionData).applicantPhone,
            animalId: (data as AdoptionData).animal?.id || "",
            animalName: (data as AdoptionData).animal?.name,
          } : null}
          onSuccess={() => {
            queryClient.invalidateQueries({ queryKey: [getQueryKey(type)] });
      ...
      {/* Move and Send Foster Agreement Dialog */}
      {type === "foster" && data && (
        <MoveAndSendFosterAgreementDialog
          open={showFosterAgreementDialog}
          onOpenChange={(open) => {
            setShowFosterAgreementDialog(open);
          }}
          application={data ? {
            id: data.id,
            applicantName: (data as FosterData).applicantName,
            applicantEmail: (data as FosterData).applicantEmail,
            applicantPhone: (data as FosterData).applicantPhone,
          } : null}
          onSuccess={() => {
            queryClient.invalidateQueries({ queryKey: [getQueryKey(type)] });
            setShowFosterAgreementDialog(false);
            onClose();
          }}
        />
      )}

      {/* Move and Send Volunteer Waiver Dialog */}
      {type === "volunteer" && data && (
        <MoveAndSendVolunteerWaiverDialog
          open={showVolunteerWaiverDialog}
          onOpenChange={(open) => {
            setShowVolunteerWaiverDialog(open);
          }}
          application={data ? {
            id: data.id,
            applicantName: (data as VolunteerData).applicantName,
            applicantEmail: (data as VolunteerData).applicantEmail,
            applicantPhone: (data as VolunteerData).applicantPhone,
          } : null}
          onSuccess={() => {
            queryClient.invalidateQueries({ queryKey: [getQueryKey(type)] });
            setShowVolunteerWaiverDialog(false);
            onClose();
          }}
        />
      )}
      ...
      {type === "adoption" && data && (
        <ChangeAnimalDialog
          ...
          currentAnimalId={(data as AdoptionData).animal?.id}
          currentAnimalName={(data as AdoptionData).animal?.name}
          applicantName={(data as AdoptionData).applicantName}
        />
      )}
    </>
