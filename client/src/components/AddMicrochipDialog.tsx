...
const microchipSchema = z.object({
  microchipNumber: z.string().min(9, "Microchip number must be at least 9 digits").max(15, "Microchip number cannot exceed 15 digits"),
  manufacturer: z.string().min(1, "Manufacturer is required"),
  implantDate: z.string().optional(),
  implantLocation: z.string().optional(),
  chipOrigin: z.string().min(1, "Chip origin is required"),
...
const MANUFACTURERS = [
  ...
  { value: "found_animals", label: "Found Animals" },
  { value: "petlink", label: "PetLink" },
  { value: "other", label: "Other" },
];

const CHIP_ORIGINS = [
  { value: "intake_with_chip", label: "Arrived with chip (intake)" },
  { value: "implanted_by_rescue", label: "Implanted by rescue" },
  { value: "found", label: "Found/stray with chip" },
];
  ...
  useEffect(() => {
    if (microchip) {
      form.reset({
        ...
        chipOrigin: microchip.chipOrigin || "implanted_by_rescue",
        registrationStatus: microchip.registrationStatus || "unregistered",
