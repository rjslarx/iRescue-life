...
import { CheckCircle2, Circle, Settings, Mail, DollarSign, PawPrint, ChevronRight, X, FileText } from "lucide-react";
...
export function OnboardingChecklist({ tenant, animalCount = 0, hasPlatformEmailKey = false }: OnboardingChecklistProps) {
  ...
  const isLite = !tenant.subscriptionTier || tenant.subscriptionTier === 'lite' || tenant.subscriptionTier === 'free';
  ...
  const items: ChecklistItem[] = [
    ...
    },
    {
      ...
      completed: tenant.resendEnabled || hasPlatformEmailKey,
  ...
  if (isLite) {
    items.push({
      id: "external-forms",
      title: "Configure External Form Links",
      description: "Set up your JotForm or Google Form URLs for adoption, foster, and volunteer applications",
      completed: !!(tenant as any).jotformAdoptionUrl || !!(tenant as any).jotformFosterUrl,
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex-1">
            ...
            <CardDescription className="mt-1">
              Complete these steps to get your {isLite ? "Lite" : "Professional"} rescue portal up and running
            </CardDescription>
          </div>
          <Button
            variant="ghost"
