...
import { Heart, Loader2, QrCode, ExternalLink, Copy, Check, DollarSign, MessageCircle, Eye } from "lucide-react";
...
export function MedicalFundDialog({ animal, trigger }: MedicalFundDialogProps) {
  ...
  const { data: tenantData } = useQuery<{ tenant: { stripeEnabled?: boolean } }>({
    queryKey: ['/api/tenant'],
    enabled: open,
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button variant="outline" size="sm" className="gap-2 justify-center" data-testid="button-medical-fund">
            ...
            <span className="hidden sm:inline">Med Fund</span>
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-lg">
        ...
        ) : !tenantData?.tenant?.stripeEnabled ? (
          ...
          <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-4 pt-4 border-t">
                    ...
                    <div className="flex flex-wrap gap-2 pt-4 border-t">
                      <Button
                        ...
                        data-testid="button-campaign-updates"
                      >
                        <MessageCircle className="h-4 w-4 mr-2" />
                        Post Updates
                      </Button>
                      {fundStatus.campaignId && (
                        <Button
                          variant="outline"
                          className="flex-1"
                          asChild
                          data-testid="button-view-campaign-page"
                        >
                          <a href={`/campaigns/${fundStatus.campaignId}`} target="_blank" rel="noopener noreferrer">
            ...
            {fundStatus.campaignId && (
              <CampaignUpdatesDialog
                ...
                campaignId={fundStatus.campaignId}
                campaignTitle={`Help ${animal.name} with medical expenses`}
              />
            )}
          </div>
        ) : (
