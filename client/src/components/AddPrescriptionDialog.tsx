...
export function AddPrescriptionDialog({ animalId, open, onOpenChange, prescription }: AddPrescriptionDialogProps) {
  ...
  const isOneTime = watchedFrequency === "ONCE";
  ...
  // When frequency is "ONCE": auto-set endDate = startDate
  useEffect(() => {
    if (isOneTime && watchedStartDate) {
      form.setValue("endDate", watchedStartDate);
    }
  }, [isOneTime, watchedStartDate, form]);

  const [userEditedNextDose, setUserEditedNextDose] = useState(false);

  useEffect(() => {
    if (open && prescription?.nextScheduledDose) {
      setUserEditedNextDose(true);
    } else {
      setUserEditedNextDose(false);
    }
  }, [open, prescription]);

  useEffect(() => {
    if (!watchedStartDate || !watchedFrequency || isOneTime || watchedFrequency === "PRN" || userEditedNextDose) return;
    ...
    const daysToAdd = frequencyDaysMap[watchedFrequency];
    if (daysToAdd) {
      const start = new Date(watchedStartDate + "T00:00:00");
      const todayDate = new Date(today + "T00:00:00");
      let nextDose = new Date(start);
      if (nextDose < todayDate) {
        while (nextDose < todayDate) {
          nextDose.setDate(nextDose.getDate() + daysToAdd);
        }
      }
      const nextDoseStr = nextDose.toISOString().split('T')[0];
      form.setValue("nextScheduledDose", nextDoseStr);
    }
  }, [watchedStartDate, watchedFrequency, isOneTime, today, form, userEditedNextDose]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{isOneTime ? "End Date (Auto-set)" : "End Date (Optional)"}</FormLabel>
                    <FormControl>
                      <div className="flex gap-2">
                        <Input type="date" {...field} data-testid="input-end-date" className="flex-1" disabled={isOneTime} />
                        {field.value && !isOneTime && (
                          <Button
                            type="button"
                            variant="ghost"
                            size="icon"
                            onClick={() => form.setValue("endDate", "")}
                    ...
                    <FormDescription className="text-xs">
                      {isOneTime ? "Automatically set to start date for one-time medications" : "Leave blank for ongoing medication"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            {!isOneTime && watchedFrequency !== "PRN" && (
              <FormField
                control={form.control}
                name="nextScheduledDose"
                render={({ field }) => (
                  <FormItem className={isStartDateInPast
                    ? "rounded-md border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950 p-4"
                    : ""
                  }>
                    <FormLabel className={isStartDateInPast ? "text-amber-800 dark:text-amber-200" : ""}>Next Dose Due</FormLabel>
                    <FormControl>
                      <div className="flex gap-2">
                        <Input type="date" {...field} onChange={(e) => { field.onChange(e); setUserEditedNextDose(true); }} data-testid="input-next-scheduled-dose" className="flex-1" />
                        {field.value && (
                          <Button
                            type="button"
                            variant="ghost"
                            size="icon"
                            onClick={() => { form.setValue("nextScheduledDose", ""); setUserEditedNextDose(false); }}
                            data-testid="button-clear-next-scheduled-dose"
                            title="Clear next due date"
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        )}
                    ...
                    <FormDescription className={isStartDateInPast ? "text-xs text-amber-700 dark:text-amber-300" : "text-xs"}>
                      Auto-calculated from start date and frequency. You can override this value if needed.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
