...
const animalFormSchema = insertAnimalSchema.omit({ tenantId: true }).extend({
  ...
  needsSpayNeuter: z.boolean().default(false),
  isFlightRisk: z.boolean().default(false),
  ...
  locationFound: z.string().optional().default(""),
  strayHoldUntil: z.date().optional().nullable(),
...
export function AnimalDataForm({ animalId, initialData, onSuccess, onCancel }: AnimalDataFormProps) {
  ...
  const fosterAutoMode = useMemo(() => {
    if (mode !== 'edit' || !animalData) return false;
    const hasActiveFoster = !!(animalData as any).activeFosterName;
    if (!hasActiveFoster) return false;
    const hasKennel = !!(animalData as any).kennelBuildingId && !!(animalData as any).kennelRowId && (animalData as any).kennelPosition !== null && (animalData as any).kennelPosition !== undefined;
    if (!hasKennel) return true;
    const kennelUpdated = animalData.updatedAt ? new Date(animalData.updatedAt).getTime() : 0;
    const fosterStart = (animalData as any).activeFosterStartDate ? new Date((animalData as any).activeFosterStartDate).getTime() : 0;
    return fosterStart > kennelUpdated;
  }, [animalData, mode]);

  const activeFosterName = useMemo(() => {
    return fosterAutoMode ? ((animalData as any)?.activeFosterName || 'Foster Parent') : '';
  }, [fosterAutoMode, animalData]);
  ...
  useEffect(() => {
    if (animalData) {
      form.reset({
        ...
        biteHistory: animalData.biteHistory ?? false,
        specialDiet: animalData.specialDiet ?? false,
        needsSpayNeuter: animalData.needsSpayNeuter ?? false,
        isFlightRisk: animalData.isFlightRisk ?? false,
        ...
        locationFound: animalData.locationFound || "",
        strayHoldUntil: animalData.strayHoldUntil ? new Date(animalData.strayHoldUntil) : null,
        locationType: (animalData as any).locationType || "shelter",
        locationName: (animalData as any).locationName || "",
      });
  ...
  const watchedIntakeSource = form.watch("intakeSource");
  ...
  const handleSubmit = (data: AnimalFormData) => {
    ...
    if (data.locationType === 'shelter') {
      const hasBuilding = !!selectedBuildingId;
      const hasRow = !!selectedRowId;
      const hasPosition = selectedPosition !== null && selectedPosition !== undefined;
      const hasAnyKennel = hasBuilding || hasRow || hasPosition;
      const hasCompleteKennel = hasBuilding && hasRow && hasPosition;
      
      if (hasAnyKennel && !hasCompleteKennel) {
        toast({
          title: "Incomplete Kennel Assignment",
          description: "Please select building, row, AND kennel # together, or leave all empty.",
          variant: "destructive",
        });
        return;
      }
    ...
    const submitData = {
      ...
      locationFound: data.locationFound || null,
      strayHoldUntil: data.strayHoldUntil ? data.strayHoldUntil.toISOString() : null,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="border-t pt-4">
          ...
          {(watchedStatus === 'stray_hold' || watchedIntakeSource === 'stray' || form.getValues('locationFound') || form.getValues('strayHoldUntil')) && (
            <div className="rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950 p-4 space-y-4 mt-4">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Stray Hold Information</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
                  name="locationFound"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Stray Hold Until</FormLabel>
                      <FormControl>
                        <Input
                          type="date"
                          data-testid="input-stray-hold-until"
                          value={field.value ? new Date(field.value).toISOString().split('T')[0] : ''}
                          onChange={(e) => {
                      ...
                      <FormDescription>Date when the stray hold period expires</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
        ...
        <div className="border-t pt-4">
          <h3 className="text-sm font-medium mb-3">Badges & Warnings</h3>
          <p className="text-sm text-muted-foreground mb-3">Toggle warning badges that appear on the animal card. These help staff quickly identify important needs and restrictions.</p>

          <div className="space-y-4">
            <div>
              <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Medical Badges</Label>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center gap-3 space-y-0">
                      ...
                      <FormLabel className="text-sm font-normal cursor-pointer">Heartworm Positive (HW+)</FormLabel>
                    </FormItem>
                  )}
                />
                <FormField
                  control={form.control}
                  name="needsSpayNeuter"
                  render={({ field }) => (
                    <FormItem className="flex items-center gap-3 space-y-0">
                      <FormControl>
                        <Checkbox
                          ...
                          data-testid="checkbox-needs-spay-neuter"
                        />
                      </FormControl>
                      <FormLabel className="text-sm font-normal cursor-pointer">Needs Spay/Neuter</FormLabel>
                    </FormItem>
                  )}
                />
                <FormField
                    ...
                    <FormItem className="flex items-center gap-3 space-y-0">
                      <FormControl>
                        <Checkbox
                          checked={field.value === true}
                          onCheckedChange={(checked) => field.onChange(!!checked)}
                          data-testid="checkbox-special-diet"
                        />
                      </FormControl>
                      <FormLabel className="text-sm font-normal cursor-pointer">Special Diet Required</FormLabel>
                    </FormItem>
                  )}
                />
              </div>
            ...
            <div>
              <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Behavioral Badges</Label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                <FormField
                  control={form.control}
                  name="biteHistory"
                  render={({ field }) => (
                    <FormItem className="flex items-center gap-3 space-y-0">
                      ...
                      <FormLabel className="text-sm font-normal cursor-pointer">Bite History</FormLabel>
                    </FormItem>
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center gap-3 space-y-0">
                      ...
                      <FormLabel className="text-sm font-normal cursor-pointer">Flight Risk (Escape Artist)</FormLabel>
                    </FormItem>
                  )}
                />
              </div>
            ...
            <div>
              <Label className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Compatibility</Label>
              <p className="text-xs text-muted-foreground mt-1 mb-2">Setting these to "No" will show a warning badge on the animal card.</p>
