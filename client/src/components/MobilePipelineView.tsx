...
import { ChevronRight, ChevronDown, ChevronUp, MoreVertical, Eye, ArrowRight, LucideIcon } from "lucide-react";
...
interface MobilePipelineViewProps {
  ...
  cappedStages?: CappedStageConfig[];
}
...
export default function MobilePipelineView({
  ...
  const [expandedCappedStages, setExpandedCappedStages] = useState<Set<string>>(new Set());

  const getCappedLimit = (stageId: string): number | undefined => {
    return cappedStages?.find(c => c.stageId === stageId)?.limit;
  };

  const toggleCappedStage = (stageId: string) => {
    setExpandedCappedStages(prev => {
    ...
    <div className="space-y-2">
      ...
      >
        {stages.map((stage) => {
          ...
          const cappedLimit = getCappedLimit(stage.id);
          const isCapped = cappedLimit !== undefined && stageCards.length > cappedLimit && !expandedCappedStages.has(stage.id);
          const visibleCards = isCapped ? stageCards.slice(0, cappedLimit) : stageCards;
              ...
              <AccordionContent className="px-2 pb-2">
                {stageCards.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {cappedLimit !== undefined && stageCards.length > cappedLimit && (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="w-full text-xs text-muted-foreground"
                        onClick={() => toggleCappedStage(stage.id)}
                        data-testid={`button-toggle-cap-${stage.id}`}
                      >
                        {expandedCappedStages.has(stage.id) ? (
                          <>
                          ...
                          <>
                            ...
                            View all {stageCards.length} {stage.label.toLowerCase()}
                          </>
                        )}
                      </Button>
                    )}
