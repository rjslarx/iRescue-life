...
export default function SetupWizard({ open, onOpenChange }: SetupWizardProps) {
  ...
  useEffect(() => {
    if (wizardStatus && !isLoading) {
      setCurrentStep(wizardStatus.wizardStep);
    }
  ...
  const handleBack = async () => {
    if (currentStep > 0) {
      ...
      await updateStepMutation.mutateAsync(prevStep);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 flex flex-col" data-testid="dialog-setup-wizard">
        <DialogHeader className="p-6 pb-0 shrink-0">
          ...
          <DialogDescription>
            Let's set up your rescue organization's website in just a few steps
          </DialogDescription>
        </DialogHeader>
