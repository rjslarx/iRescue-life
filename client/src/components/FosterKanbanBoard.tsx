...
function shouldShowPendingAgreement(app: FosterApplication): boolean {
  const stage = normalizeStage(app.stage);
  if (stage !== 'agreement' && stage !== 'active_pool') return false;
  if (!app.agreementStatus) return true;
  const s = app.agreementStatus.status;
...
export default function FosterKanbanBoard({ 
  ...
  const [showAllActivePool, setShowAllActivePool] = useState(false);
  const isMobile = useIsMobile();

  const getApplicationsByStage = (stageId: string) => {
    const apps = applications.filter(app => normalizeStage(app.stage) === stageId);
    if (stageId === 'active_pool') {
      apps.sort((a, b) => {
  ...
  const appToCard = (app: FosterApplication): PipelineCard => {
    ...
    if (shouldShowPendingAgreement(app)) {
      badges.push({ label: "Pending Agreement", variant: "outline" });
    } else if (app.agreementStatus) {
  ...
  if (isMobile) {
    return (
      <MobilePipelineView
        ...
        cappedStages={[{ stageId: 'active_pool', limit: ACTIVE_POOL_VISIBLE_LIMIT }]}
      />
    );
  }
                  ...
                  <ScrollArea className="h-[400px] pr-4">
                    {stageApps.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {(stage.id === 'active_pool' && !showAllActivePool && stageApps.length > ACTIVE_POOL_VISIBLE_LIMIT
                          ? stageApps.slice(0, ACTIVE_POOL_VISIBLE_LIMIT)
                          : stageApps
                        ).map((app) => (
                          <Card
                            key={app.id}
                            ...
                            <CardContent className="p-4 space-y-2">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex flex-col gap-1 items-end">
                                  {shouldShowPendingAgreement(app) ? (
                                    <Badge variant="outline" className="text-xs gap-1 shrink-0 text-amber-600 border-amber-300" data-testid="badge-pending-agreement">
                        ...
                        {stage.id === 'active_pool' && stageApps.length > ACTIVE_POOL_VISIBLE_LIMIT && (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="w-full text-muted-foreground"
                            onClick={() => setShowAllActivePool(!showAllActivePool)}
                            data-testid="button-view-all-active-pool"
                          >
                            {showAllActivePool ? (
                              <>
                              ...
                              <>
                                ...
                                View all {stageApps.length} active fosters
                              </>
                            )}
                          </Button>
                        )}
