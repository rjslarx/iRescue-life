...
- **Daily Briefing Timezone Handling:** The Command Center Daily Briefing module displays surgeries, transports, medical tasks, and calendar events for "today" and "tomorrow". Server runs in UTC but dates are displayed in user's local time. To account for US timezone offsets (up to 8 hours behind UTC), queries extend the date range by +1 day. A surgery stored as "Feb 6 00:00 UTC" appears as "Feb 5 6PM CST" to users. Future enhancement: add tenant-specific timezone configuration for more accurate date boundary calculations.
