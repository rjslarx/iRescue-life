...
// ============================================================
// Foster Compliance Reminder Endpoints
// ============================================================

/**
 * POST /api/compliance/confirm/:token
 * Confirm a foster compliance reminder via magic link
 * This is a public endpoint (no auth required) since it's accessed via email/SMS links
 */
router.post('/confirm/:token', async (req, res, next) => {
  try {
    ...
    const { processConfirmation } = await import('../lib/foster-compliance-reminders');
    const result = await processConfirmation(token, 'magic_link');
    ...
    if (result.alreadyConfirmed) {
      return res.json({ 
        ...
        message: 'This item was already confirmed',
        confirmedAt: result.reminder?.confirmedAt,
      });
    }
    ...
    if (!result.success) {
      return res.status(500).json({ error: result.error || 'Failed to confirm' });
    }
 ...
 * Get reminder details for confirmation page (public endpoint)
 */
router.get('/confirm/:token', async (req, res, next) => {
  try {
    ...
    const { complianceReminders, animals } = await import('@shared/schema');
    ...
    const [reminder] = await db
      .select({
        ...
        state: complianceReminders.state,
        confirmedAt: complianceReminders.confirmedAt,
        magicLinkExpiresAt: complianceReminders.magicLinkExpiresAt,
    ...
    const isExpired = reminder.magicLinkExpiresAt && new Date() > reminder.magicLinkExpiresAt;
    const isConfirmed = reminder.state === 'confirmed';
...
/**
 * GET /api/compliance/foster/stats
 * Get foster compliance statistics for the Compliance Widget
 */
router.get('/foster/stats', requireTenant, requireAuth, async (req, res, next) => {
  try {
    const { getComplianceStats } = await import('../lib/foster-compliance-reminders');
    const stats = await getComplianceStats(req.tenant!.id);
    res.json(stats);
  } catch (error) {
    next(error);
  }
...
/**
 * GET /api/compliance/foster/call-list
 * Get the call list of reminders requiring staff intervention
 */
router.get('/foster/call-list', requireTenant, requireAuth, async (req, res, next) => {
  try {
    if (!['admin', 'staff', 'manager'].includes(req.user!.activeRole || '')) {
      return res.status(403).json({ error: 'Access denied' });
    }

    const { getCallList } = await import('../lib/foster-compliance-reminders');
    const result = await getCallList(req.tenant!.id);
...
/**
 * POST /api/compliance/foster/call-list/:reminderId/resolve
 * Resolve a call list item after staff has contacted the foster
 */
router.post('/foster/call-list/:reminderId/resolve', requireTenant, requireAuth, async (req, res, next) => {
  try {
    if (!['admin', 'staff', 'manager'].includes(req.user!.activeRole || '')) {
      return res.status(403).json({ error: 'Access denied' });
    }
    ...
    const { notes, markAsConfirmed = true } = req.body;

    const { resolveCallListItem } = await import('../lib/foster-compliance-reminders');
    const result = await resolveCallListItem(reminderId, req.user!.id, notes, markAsConfirmed);
    
    if (!result.success) {
      return res.status(500).json({ error: result.error });
    }

    res.json({ success: true, message: 'Call list item resolved' });
  } catch (error) {
    next(error);
  }
});
...
/**
 * POST /api/compliance/foster/call-list/batch-resolve
 * Batch resolve multiple call list items
 */
router.post('/foster/call-list/batch-resolve', requireTenant, requireAuth, async (req, res, next) => {
  try {
    if (!['admin', 'staff', 'manager'].includes(req.user!.activeRole || '')) {
      return res.status(403).json({ error: 'Access denied' });
    }

    const { reminderIds, notes, markAsConfirmed = true } = req.body;
    ...
    for (const reminderId of reminderIds) {
      ...
      if (result.success) {
        ...
        errors.push(`${reminderId}: ${result.error}`);
      }
    }
...
/**
 * POST /api/compliance/foster/test-trigger
 * Manual trigger for testing the foster compliance reminder flow
 * Admin only - runs the reminder jobs on-demand without waiting for scheduled times
 */
router.post('/foster/test-trigger', requireTenant, requireAuth, async (req, res, next) => {
  try {
    if (!['admin', 'platform_admin'].includes(req.user!.activeRole || '')) {
    ...
    const {
      createRemindersForDuePreventativeCare,
      send3DayWarningEmails,
      sendMorningOfSmsReminders,
      escalateOverdueReminders,
    } = await import('../lib/foster-compliance-reminders');

    let result: { success: boolean; message: string; details?: any };

    switch (action) {
      case 'create_reminders':
        const createResult = await createRemindersForDuePreventativeCare(tenantId);
        result = {
          success: true,
          message: `Created ${createResult.created} reminders`,
          details: createResult,
        };
        break;

      case 'send_3day_emails':
        const emailResult = await send3DayWarningEmails(tenantId);
        result = {
          success: true,
          message: `Sent ${emailResult.sent} 3-day warning emails`,
          details: emailResult,
        };
        break;

      case 'send_morning_sms':
        const smsResult = await sendMorningOfSmsReminders(tenantId);
        result = {
          success: true,
          message: `Sent ${smsResult.sent} morning-of SMS reminders`,
          details: smsResult,
        };
        break;

      case 'escalate_overdue':
        const escalateResult = await escalateOverdueReminders(tenantId);
        result = {
          success: true,
          message: `Escalated ${escalateResult.escalated} overdue reminders`,
          details: escalateResult,
        };
        break;

      case 'run_all':
        const allCreateResult = await createRemindersForDuePreventativeCare(tenantId);
        const allEmailResult = await send3DayWarningEmails(tenantId);
        const allSmsResult = await sendMorningOfSmsReminders(tenantId);
        const allEscalateResult = await escalateOverdueReminders(tenantId);
        result = {
          success: true,
          message: 'Ran all foster compliance jobs',
          details: {
            remindersCreated: allCreateResult.created,
            emailsSent: allEmailResult.sent,
            smsSent: allSmsResult.sent,
            escalated: allEscalateResult.escalated,
            errors: [
              ...allCreateResult.errors,
              ...allEmailResult.errors,
              ...allSmsResult.errors,
              ...allEscalateResult.errors,
            ],
          },
        };
        break;
