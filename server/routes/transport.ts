...
// ============================================================================
// Driver Packet - Printable Document Assembly
// ============================================================================

router.get('/events/:transportId/driver-packet', requireTenant, requireAuth, async (req, res, next) => {
  try {
    const tenantId = req.tenant!.id;
    const transportId = req.params.transportId;

    const transport = await TransportService.getTransport(tenantId, transportId);
    if (!transport) {
      return res.status(404).json({ error: 'Transport not found' });
    }

    const participants = await TransportService.listParticipants(tenantId, transportId);
    const stops = await TransportService.listStops(tenantId, transportId);
    const manifestItems = await TransportService.listManifestItems(tenantId, transportId);

    const [tenant] = await db.select({
      name: tenants.name,
      phone: tenants.contactPhone,
      email: tenants.contactEmail,
    }).from(tenants).where(eq(tenants.id, tenantId)).limit(1);

    const animalsWithMedical = await Promise.all(
      manifestItems.map(async (item) => {
        ...
        const microchips = await db.select()
    ...
    res.json({
      ...
      participants,
      stops,
      manifest: animalsWithMedical,
      organization: tenant || { name: 'Unknown', phone: null, email: null },
    });
  } catch (error) {
    next(error);
  }
