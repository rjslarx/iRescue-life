...
export class StorageBackupService {
  ...
  private async backupVolunteerDocuments(tenantId: string, driveService: DriveService, progress: BackupProgress): Promise<void> {
    ...
    try {
      // Get volunteers with signed waivers
      const volunteers = await db
        .select({
          id: volunteerApplications.id,
          name: volunteerApplications.name,
          signedWaiverUrl: volunteerApplications.signedWaiverUrl,
      ...
      for (const volunteer of volunteers) {
        ...
        if (volunteer.signedWaiverUrl.includes('drive.google.com') || volunteer.signedWaiverUrl.includes('googleusercontent.com')) {
          progress.filesSkipped++;
          continue;
        }

        if (!volunteer.signedWaiverUrl.startsWith('/objects/')) {
          progress.filesSkipped++;
          continue;
        }

        try {
          // Create volunteer folder if needed
          ...
          const waiversFolder = await this.getOrCreateFolder(driveService, 'Waivers', volunteerFolder.folderId);
          if (!waiversFolder.success || !waiversFolder.folderId) continue;
