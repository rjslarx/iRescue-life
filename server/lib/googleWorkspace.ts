...
export class CalendarService {
  ...
  async listEvents(googleCalendarId: string, timeMin: Date, timeMax: Date): Promise<{
    success: boolean;
    events?: Array<{
      id: string;
      summary: string;
    ...
    try {
      ...
      const cal = google.calendar({ version: 'v3', auth: this.service.getOAuth2Client() });
      ...
      console.error(`[CalendarService] Error listing events from ${googleCalendarId}:`, error.message);
      return {
        success: false,
