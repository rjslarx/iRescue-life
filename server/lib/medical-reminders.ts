  ...
  const activeAnimals = await db
    ...
    .where(and(
      ...
      or(
        ...
        eq(animals.status, "transfer_pending"),
        eq(animals.status, "medical_hold"),
        eq(animals.status, "stray_hold")
      )
    ));
...
export async function generateMedicalReminderEmail(
  ...
  const renderItem = (item: MedicalItem, isOverdue: boolean = false) => `
    <tr style="border-bottom: 1px solid #e5e7eb;">
      ...
      <td style="padding: 12px 8px;">${item.fosterName ? `Foster: ${item.fosterName}` : 'In Shelter'}</td>
    </tr>
  `;
  ...
  const records = await db
    .select({
      ...
      animal: {
        ...
        location: animalsTable.locationType,
      },
    })
    ...
    .where(and(
      ...
      or(
        ...
        eq(animalsTable.status, 'adoption_pending'),
        eq(animalsTable.status, 'transfer_pending'),
        eq(animalsTable.status, 'medical_hold'),
        eq(animalsTable.status, 'stray_hold')
      )
    ))
