...
import { users, userPagePermissions } from '@shared/schema';
import { db } from '../db';
    ...
    try {
      ...
      if (user) {
        ...
        if (req.session.impersonatingUserId && req.session.realUserId) {
          ...
          if (impersonatedUser) {
            // Determine active role: use session's activeRole if valid for impersonated user, otherwise default to first role
            let activeRole = req.session.activeRole;
            if (!activeRole || !impersonatedUser.roles.includes(activeRole)) {
              activeRole = impersonatedUser.roles[0] || 'volunteer';
...
/**
 * Middleware that checks role OR explicit page permission grants.
 * If the user's activeRole passes the standard requireRole check, they proceed.
 * Otherwise, checks the user_page_permissions table for an explicit grant on the given pageId.
 * This bridges the gap between page-level navigation permissions and API data access.
 */
export function requireRoleOrPageAccess(pageId: string, ...roles: string[]) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Special case: 'owner' role
    if (roles.includes('owner') && req.user.roles.includes('owner')) {
      return next();
    }

    // Finally, check user_page_permissions for explicit grant
    if (req.tenant) {
      try {
        ...
        // If page permissions check fails, fall through to deny
      }
    }
...
export function requireProTier(req: Request, res: Response, next: NextFunction) {
  ...
  const tier = (req.tenant as any).subscriptionTier;
  if (tier === 'lite') {
    return res.status(403).json({
      error: 'Pro Feature',
      message: 'This feature requires the Professional plan. Please upgrade to access this functionality.',
    });
  }
