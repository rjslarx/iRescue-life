...
export const STANDALONE_MERGE_FIELDS = {
  ...
  '{{date}}': 'Current date (full format)',
  '{{date_day}}': 'Day of month (e.g., 10)',
  '{{date_month}}': 'Month name (e.g., February)',
  '{{date_year}}': 'Two-digit year (e.g., 26)',
  '{{organization_name}}': 'Organization name',
  '{{signature_image_url}}': 'Signature image (embedded)',
...
export async function renderFormHtml(
  ...
  const signingDate = submission.signedAt || new Date();
  const replacements: Record<string, string> = {
    '{{signer_name}}': submission.signerName || '',
    '{{signer_email}}': submission.signerEmail || '',
    '{{signer_phone}}': submission.signerPhone || '',
    '{{date}}': signingDate.toLocaleDateString('en-US', { 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    }),
    '{{date_day}}': String(signingDate.getDate()),
    '{{date_month}}': signingDate.toLocaleString('en-US', { month: 'long' }),
    '{{date_year}}': String(signingDate.getFullYear()).slice(-2),
    '{{organization_name}}': tenantName,
    '{{signed_timestamp}}': submission.signedAt?.toISOString() || new Date().toISOString(),
