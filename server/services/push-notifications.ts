...
export class PushNotificationService {
  ...
  static async subscribe(
    ...
    // Check if this endpoint is registered to a different user/tenant and remove it
    const otherUserSub = await db.query.pushSubscriptions.findFirst({
  ...
  static async sendToTenantRoles(
    ...
    // Get all subscriptions for users with specified roles in the tenant
    ...
    const targetSubs = subs.filter((sub: any) => 
      sub.user && sub.user.roles.some((role: string) => roles.includes(role))
    );
