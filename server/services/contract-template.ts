...
export const MERGE_FIELDS = {
  ...
  '{{adopter_street_address}}': 'Adopter street address',
  '{{adopter_street_address_2}}': 'Adopter address line 2',
  '{{adopter_city}}': 'Adopter city',
  '{{adopter_state}}': 'Adopter state',
  '{{adopter_zip}}': 'Adopter ZIP code',
  ...
  // Medical (including smart fields)
  '{{vet_appointment_date}}': 'Vet appointment / shots due date (legacy)',
  '{{spay_neuter_date}}': 'Spay/neuter by date',
  '{{spay_status}}': 'Smart: Shows "Completed on [date]" or "Due by [date]"',
  '{{rabies_expiry}}': 'Rabies vaccination expiry date',
  '{{medical_notes}}': 'Special medical conditions/notes',
  '{{rabies_due_date}}': 'Rabies vaccine next due date',
  '{{dhpp_due_date}}': 'DHPP vaccine next due date',
  '{{bordetella_due_date}}': 'Bordetella vaccine next due date',
  '{{heartworm_due_date}}': 'Heartworm prevention next dose date',
  '{{flea_tick_due_date}}': 'Flea/tick prevention next dose date',
  // Financial
...
<body>
  ...
  <ol class="terms-list">
    ...
    <li><strong>Medical Care:</strong> The Adopter agrees to provide veterinary care as needed, including vaccinations and preventative treatments on the schedule below. The Adopter will ensure the following are administered by the dates listed:
      <ul style="margin-top: 8px; list-style-type: disc;">
        <li>Rabies vaccine due by: <strong><u>{{rabies_due_date}}</u></strong></li>
        <li>DHPP vaccine due by: <strong><u>{{dhpp_due_date}}</u></strong></li>
        <li>Bordetella vaccine due by: <strong><u>{{bordetella_due_date}}</u></strong></li>
        <li>Heartworm prevention next dose: <strong><u>{{heartworm_due_date}}</u></strong></li>
        <li>Flea/tick prevention next dose: <strong><u>{{flea_tick_due_date}}</u></strong></li>
      </ul>
    </li>
    <li><strong>Spay/Neuter:</strong> If not already done, I will have the dog spayed/neutered by <strong><u>{{spay_neuter_date}}</u></strong> and will immediately provide proof of this procedure to {{organization_name}}.</li>
...
export interface MergeData {
  ...
  rabies_due_date?: string;
  dhpp_due_date?: string;
  bordetella_due_date?: string;
  heartworm_due_date?: string;
  flea_tick_due_date?: string;
  // Financial
  adoption_fee?: string;
...
export function mergePlaceholders(htmlTemplate: string, data: Partial<MergeData>): string {
  ...
  // Add smart computed fields
  const enrichedData: Partial<MergeData> = { ...data };
  
  // {{today}} - current date in readable format
  if (!enrichedData.today) {
    enrichedData.today = new Date().toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
  ...
  for (const [key, value] of Object.entries(enrichedData)) {
    // Skip image URLs - handle separately
    if (key === 'signature_image_url' || key === 'drivers_license_image_url') continue;
  ...
  // Special handling for driver's license image URL - validate AND escape
  if (data.drivers_license_image_url) {
    const url = data.drivers_license_image_url;
    if (isSafeUrl(url)) {
      const escapedUrl = escapeHtml(url);
      result = result.replaceAll('{{drivers_license_image_url}}', escapedUrl);
    } else {
      result = result.replaceAll('{{drivers_license_image_url}}', '');
    }
  } else {
    result = result.replaceAll('{{drivers_license_image_url}}', '');
  }
