...
export async function generateMedicalHistoryPDF(animalId: string, tenantId: string): Promise<Buffer> {
  ...
  const exams = await db
    .select({
      ...
      performedBy: medicalExams.performedBy,
      subjective: medicalExams.subjective,
      objective: medicalExams.objective,
      assessment: medicalExams.assessment,
      plan: medicalExams.plan,
    })
    .from(medicalExams)
  ...
  const procedures = await db
    .select({
      ...
      veterinarian: procedureLogs.veterinarian,
  ...
  const bills = await db
    .select({
      ...
      description: medicalBills.description,
    })
    .from(medicalBills)
...
<body>
    ...
    ` : data.exams.map(exam => `
      <div class="record-card">
        ...
        <div class="record-content">
          ${exam.performedBy ? `<p><strong>Veterinarian:</strong> ${exam.performedBy}</p>` : ''}
          ${exam.subjective ? `<p><strong>Subjective:</strong> ${exam.subjective}</p>` : ''}
          ${exam.objective ? `<p><strong>Objective:</strong> ${exam.objective}</p>` : ''}
          ${exam.assessment ? `<p><strong>Assessment:</strong> ${exam.assessment}</p>` : ''}
          ${exam.plan ? `<p><strong>Plan:</strong> ${exam.plan}</p>` : ''}
        </div>
      </div>
    `).join('')}
  </div>
      ...
      <table>
        ...
        <tbody>
          ${data.vaccines.map(vax => `
            <tr>
              <td>${new Date(vax.dateGiven).toLocaleDateString()}</td>
              <td>${vax.itemName}</td>
              <td>${vax.administeredBy || 'Not specified'}</td>
              <td>${vax.lotNumber || 'N/A'}</td>
              <td>${vax.dateDue ? new Date(vax.dateDue).toLocaleDateString() : 'N/A'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  <div class="section">
    ...
    ` : data.diagnostics.map(diag => `
      <div class="record-card">
        <div class="record-header">
          <span class="record-type">${diag.testName.replace(/_/g, ' ').toUpperCase()}</span>
          <span class="record-date">${new Date(diag.testDate).toLocaleDateString()}</span>
        </div>
        <div class="record-content">
          ${diag.result ? `<p><strong>Result:</strong> ${diag.result}</p>` : ''}
        </div>
      </div>
    `).join('')}
  </div>
  ...
  <div class="section">
    ...
    ` : data.procedures.map(proc => `
      <div class="record-card">
        ...
        <div class="record-content">
          ${proc.veterinarian ? `<p><strong>Veterinarian:</strong> ${proc.veterinarian}</p>` : ''}
        ...
        <tbody>
          ${data.prescriptions.map(rx => `
            <tr>
              ...
              <td>${rx.frequency}</td>
              <td>${rx.endDate ? new Date(rx.endDate).toLocaleDateString() : 'Ongoing'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
