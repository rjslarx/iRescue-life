...
export class TransportService {
  ...
  static async updateTransport(
    ...
    const needsFinalization = data.status === 'completed' && data.status !== existing.status;

    const result = await db.transaction(async (tx) => {
      const [updated] = await tx.update(transportEvents)
        .set({
          ...data,
          updatedAt: new Date(),
        })
      ...
      if (data.status && data.status !== existing.status) {
        await tx.insert(transportUpdates).values({
          tenantId,
          transportId: id,
          updateType: 'status_change',
        ...
        if (needsFinalization) {
          await this.finalizeTransportAnimals(tenantId, updated, userId, tx);
        }
      }
  ...
  static async finalizeTransportAnimals(
    tenantId: string,
    ...
    const animalIds = transport.animalIds || [];
    if (animalIds.length === 0) return;

    const arrivalDate = transport.actualArrivalDate || transport.estimatedArrivalDate || new Date();
    const destination = transport.destinationLocation || 'Unknown destination';
    const partnerOrg = transport.partnerOrganizationName;

    for (const animalId of animalIds) {
      await executor.update(animals)
        .set({
          status: 'transported',
          kennelLocation: destination,
        })
        .where(and(
      ...
      const noteLines = [
        `Transported via "${transport.name}"`,
        `Destination: ${destination}`,
        partnerOrg ? `Partner Organization: ${partnerOrg}` : null,
        `Transport completed: ${arrivalDate instanceof Date ? arrivalDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`,
      ].filter(Boolean).join('\n');

      if (userId) {
        await executor.insert(animalNotes).values({
          tenantId,
          animalId,
          userId,
          noteText: noteLines,
  ...
  static async getTimelineEvents(
    tenantId: string,
    transportId: string
  ): Promise<(TransportTimelineEvent & { user?: { firstName: string | null; lastName: string | null } | null })[]> {
